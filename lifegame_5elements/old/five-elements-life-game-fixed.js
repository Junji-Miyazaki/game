...
let yearTotalTime = 180;    // 1年の合計時間（3分 = 180秒）
let seasonTime = 36;        // 各季節の時間（36秒）
...
class Virus extends Entity {
    constructor(x, y, type) {
        ...
        if (type === 'lethal') {
            // 災害ウイルスは季節に関係なく強力
            infectionRateModifier = 2.0;
            lethalityRateModifier = 2.0;
    ...
    move() {
        ...
        if ((this.type === 'yin' && (currentSeason === 'water' || currentSeason === 'metal')) ||
            ...
            lifespanDecreaseRate = 0.25; // 相性の悪い季節では短命
        }
                        ...
                        if (neighbor.type === 'wood' || neighbor.type === 'water') {
                            ...
                            for (const secNeighbor of secondaryNeighbors) {
                                if (secNeighbor instanceof Cell && !secNeighbor.immunity && 
                                    ...
                                    // 季節による調整
                                    if (neighbor.type === currentSeason) {
                                        spreadChance = 0.3; // その季節は感染拡大も抑制
                                    } else if (getOverridingElement(neighbor.type) === currentSeason) {
                                        spreadChance = 0.8; // 相剋される要素は感染拡大しやすい
                                    }
...
class Cell extends Entity {
    ...
    move() {
        ...
        if (this.infected > 0) {
            ...
            if (this.type === currentSeason) {
                energyDrainModifier = 0.7; // 自分の季節ではエネルギー消費低下
                lifespanDrainModifier = 0.6; // 自分の季節では寿命減少が緩やか
            } else if (getOverridingElement(currentSeason) === this.type) {
                energyDrainModifier = 1.5; // 相剋関係ではエネルギー消費上昇
                lifespanDrainModifier = 1.5; // 相剋関係では寿命急減
            }
            ...
            // 季節による回復率調整
            if (this.type === currentSeason) {
                recoveryChance *= 1.5; // 自分の季節では回復率上昇
            } else if (getOverridingElement(currentSeason) === this.type) {
                recoveryChance *= 0.7; // 相剋関係では回復率低下
            }
        ...
        // 季節による繁殖しやすさの調整
        if (this.type === currentSeason) {
            reproductionThreshold *= 0.8; // 自分の季節では繁殖に必要なエネルギー低下
            reproductionProbability *= 1.5; // 自分の季節では繁殖確率上昇
        } else if (getOverridingElement(currentSeason) === this.type) {
            reproductionThreshold *= 1.2; // 相剋関係では繁殖に必要なエネルギー上昇
            reproductionProbability *= 0.7; // 相剋関係では繁殖確率低下
        }
    ...
    if (totalPopulation > 0) {
        ...
        if (disasterActive) {
            populationBarsHTML += `<div style="margin-top: 5px; color: red; font-weight: bold;">
                ! DISASTER ACTIVE: ${disasterTimer} turns remaining
            </div>`;
        }
...
function getCellResistance(type) {
    switch(type) {
        case 'metal': return 0.8; // 金属は最も高い抵抗力
        case 'wood': return 0.2;  // 木は低い抵抗力
        case 'water': return 0.3; // 水は低い抵抗力
        case 'fire': return 0.5;  // 火は中程度の抵抗力
        case 'earth': return 0.6; // 土は高めの抵抗力
        default: return 0.4;
    }
}
...
function startDisaster() {
    if (!disasterActive) {
        ...
        // 致死性ウイルスの追加
        const disasterVirusCount = 20; // 災害時のウイルス数
        
        for (let i = 0; i < disasterVirusCount; i++) {
            // ランダムな位置から生成
