...
export function drawFighter(ctx, o) {
  ...
  // SPINE / TRUNK tilt — arches BACK on the take-back, snaps FORWARD through impact, deepest at
  // the follow-through (残心), then returns to neutral. +deg = lean toward the facing direction.
  let spine = 0;
  if (atk >= 0) {
    const eOut = x => 1 - (1 - x) * (1 - x), eIn = x => x * x;
    let deg;
    if (atk < 0.25)      deg = -13 * eOut(Math.min(1, atk / 0.10));     // take-back: snap back, hold
    ...
    else                 deg = 24 * (1 - eIn((atk - 0.7) / 0.3));       // 残心: hold, then return to neutral
...
function drawSwordArm(ctx, sx, sy, atk, t, face, s, skin, gold, god, cast, restOut, front, back) {
  ...
  const KF = {
    ...
    // back view (face=-1): keep the whole cut on the sword-arm's OWN (right) side so the arm
    // never crosses in front of the back — take-back up-right, chop down the right side. (local
    // -x maps to screen +x here.)
    strike: front ? { h: [1, 11],   w: 0.00, bd:  1 } : back ? { h: [-13, -5], w: 0.00, bd: 1 } : { h: [27, 9], w: -1.45, bd: 1 },
    down:   front ? { h: [25, 15],  w: 0.00, bd:  1 } : back ? { h: [-9, 20],  w: 0.00, bd: 1 } : { h: [19, 23], w: 0.00, bd: 1 },
  ...
  else                { A = KF.down;  B = KF.idle;   const p = (atk - .70) / .30; f = p * p; }   // 残心: hold, then return
...
function drawShieldArm(ctx, sx, sy, atk, t, face, s, shB) {
  ...
  else if (atk < .25) { A = KF.idle; B = KF.wind; const x = Math.min(1, atk / .10); f = 1 - (1 - x) * (1 - x); }  // punch forward, hold
