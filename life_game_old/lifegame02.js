...
let pandemicCooldown = 0; // パンデミック冷却期間カウンター
...
// ここに getGridSizeAdjustedParams 関数を挿入
function getGridSizeAdjustedParams() {
    // グリッドのセル数（総面積）を計算
    const totalCells = GRID_SIZE_X * GRID_SIZE_Y;
    
    // 大きいグリッドほど突然変異確率は下げる（スケールを合わせる）
    const mutationScaleFactor = Math.min(1.0, 1000 / totalCells);
    
    // パンデミック終了時の冷却時間は大きいグリッドほど長く
    const cooldownDuration = Math.min(200, Math.max(50, Math.floor(totalCells / 100)));
    
    // 危険ウイルス閾値は大きいグリッドほど多く
    const dangerousVirusThreshold = Math.max(3, Math.floor(totalCells / 2000));
    
    return {
        mutationScaleFactor,
        cooldownDuration,
        dangerousVirusThreshold
    };
}

// 細胞密集地を探す新しいヘルパー関数
function findHighCellDensityDirection(x, y, searchRadius) {
    const sectors = [
        {dx: -1, dy: -1, count: 0}, // 左上
        {dx: 0, dy: -1, count: 0},  // 上
        {dx: 1, dy: -1, count: 0},  // 右上
        {dx: -1, dy: 0, count: 0},  // 左
        {dx: 1, dy: 0, count: 0},   // 右
        {dx: -1, dy: 1, count: 0},  // 左下
        {dx: 0, dy: 1, count: 0},   // 下
        {dx: 1, dy: 1, count: 0}    // 右下
    ];
        ...
        for (let dx = -searchRadius; dx <= searchRadius; dx++) {
            ...
            if (nx >= 0 && nx < GRID_SIZE_X && ny >= 0 && ny < GRID_SIZE_Y) {
                if (grid[ny][nx] instanceof Cell) {
                    // どのセクターかを決定
                    const sectorIdx = getSectorIndex(dx, dy);
                    if (sectorIdx >= 0) {
                        sectors[sectorIdx].count++;
                    }
                }
            }
        }
    ...
    for (const sector of sectors) {
        if (sector.count > maxCount) {
            maxCount = sector.count;
            bestSector = sector;
        }
    }
    
    // 細胞がどこにもない場合はnullを返す
    return maxCount > 0 ? bestSector : null;
}

// どのセクターに属するかを決定するヘルパー関数
function getSectorIndex(dx, dy) {
    if (dx < 0 && dy < 0) return 0; // 左上
    if (dx === 0 && dy < 0) return 1; // 上
    if (dx > 0 && dy < 0) return 2; // 右上
    if (dx < 0 && dy === 0) return 3; // 左
    if (dx > 0 && dy === 0) return 4; // 右
    if (dx < 0 && dy > 0) return 5; // 左下
    if (dx === 0 && dy > 0) return 6; // 下
    if (dx > 0 && dy > 0) return 7; // 右下
    return -1; // 中央（自分自身）
}
...
// パンデミック開始判定を調整
function updatePandemicState() {
    const { dangerousVirusThreshold, cooldownDuration } = getGridSizeAdjustedParams();
    
    // 危険なウイルスをカウント
    const mutantCount = viruses.filter(v => v.type === 'mutant').length;
    const lethalCount = viruses.filter(v => v.type === 'lethal').length;
    const dangerousVirusCount = mutantCount + lethalCount;
    
    // パンデミックではなく、閾値を超える危険なウイルスがいる場合はパンデミック開始
    if (!isPandemic && pandemicCooldown <= 0 && dangerousVirusCount >= dangerousVirusThreshold) {
        isPandemic = true;
        pandemicTimer = 200;
        console.log(`PANDEMIC STARTED! Dangerous viruses: ${dangerousVirusCount}`);
    }
    
    // パンデミック中だが危険なウイルスがいなくなった場合は即終了
    if (isPandemic && dangerousVirusCount === 0) {
        isPandemic = false;
        pandemicCooldown = cooldownDuration;
        console.log("Pandemic ended: All dangerous viruses eliminated");
    }
}
...
function getCellsInRadius(centerX, centerY, radius) {
    ...
    for (let dy = -radius; dy <= radius; dy++) {
        for (let dx = -radius; dx <= radius; dx++) {
            ...
            // グリッド範囲内かつ距離が半径以内かチェック
            if (x >= 0 && x < GRID_SIZE_X && y >= 0 && y < GRID_SIZE_Y) {
                const distance = Math.sqrt(dx*dx + dy*dy);
                if (distance <= radius && grid[y][x] instanceof Cell) {
                    cellsInRange.push(grid[y][x]);
                }
            }
        }
    }
...
class Virus extends Entity {
    constructor(x, y, type) {
        ...
        // 寿命を延長
        this.baseLifespan = 300; // 40→300に増加
        ...
        // グリッドサイズに応じた突然変異率調整
        const { mutationScaleFactor } = getGridSizeAdjustedParams();
        this.mutationChance = 0.000005 * mutationScaleFactor;
    }
    ...
    move() {
        ...
        if (this.type === 'yin') {
            ...
            // 陽ウイルスは広範囲移動 - 大幅に移動距離増加
            const moveRange = 7; // 移動範囲を3→7に拡大
            
            // 10%の確率で大きなジャンプ、それ以外は通常移動
            if (random < 0.1) {
                // 大きなジャンプ（10〜15マス）
                const jumpDistance = 10 + Math.floor(Math.random() * 6);
                const angle = Math.random() * Math.PI * 2; // ランダムな方向
                dx = Math.round(Math.cos(angle) * jumpDistance);
                dy = Math.round(Math.sin(angle) * jumpDistance);
            } else {
                // 通常の広範囲移動（最大7マス）
            ...
            // 小確率でターゲット（細胞密集地）に向かう
            if (random < 0.15) {
                // 16マス範囲内で最も細胞が密集している方向を探す
                const cellDensityDirection = findHighCellDensityDirection(this.x, this.y, 16);
                if (cellDensityDirection) {
                    dx = cellDensityDirection.dx * moveRange;
                    dy = cellDensityDirection.dy * moveRange;
                }
            }
            ...
            if (random < 0.4) {
                // 細胞密集地を探して移動
                const cellDensityDirection = findHighCellDensityDirection(this.x, this.y, 8);
                if (cellDensityDirection) {
                    dx = cellDensityDirection.dx * 2;
                    dy = cellDensityDirection.dy * 2;
                } else {
                    dx = Math.floor(Math.random() * 3) - 1;
                    dy = Math.floor(Math.random() * 3) - 1;
                }
        ...
        switch(this.type) {
            ...
            case 'yang': reproductionChance = 0.007; break; // 増加
            case 'mutant': reproductionChance = 0.004; break;
            case 'lethal': reproductionChance = 0.002; break;
        }
    ...
    tryInfection() {
        // 基本の感染範囲を取得
        let infectionRadius = 1; // デフォルトは周囲1マス
        
        // 陽ウイルスは広範囲の感染力を持つ
        if (this.type === 'yang') {
            infectionRadius = 3; // 周囲3マスに拡大
        }
        
        // 指定半径内の細胞を取得
        const cellsInRange = getCellsInRadius(this.x, this.y, infectionRadius);
        ...
        for (const cell of cellsInRange) {
            if (!cell.immunity) {
                ...
                // 距離に基づいて感染率を調整（陽ウイルスのみ）
                if (this.type === 'yang') {
                    const distance = Math.sqrt(Math.pow(cell.x - this.x, 2) + Math.pow(cell.y - this.y, 2));
                    const distanceFactor = 1 - (distance / (infectionRadius + 1));
                    infectionChance *= distanceFactor;
                }
                ...
                if (Math.random() < infectionChance) {
                    if (!cell.infected) {
                        cell.infected = 20; // 20ターンの感染
                        cell.infectedBy = this.type; // 感染源を記録
                        ...
                        if (this.type === 'yang') {
                            ...
                            for (const neighbor of neighborCells) {
                                if (neighbor instanceof Cell) {
                                    let relation = getFiveElementsRelation(cell.type, neighbor.type, neighbor.immuneToOverride);
    ...
    draw() {
        ...
        if (this.type === 'mutant') {
            ...
            finalSize = size * 1.1; // 陽ウイルスは少し大きく
        }
        ...
        if (this.type === 'yang') {
            ...
            // エネルギー波紋効果
            const pulseSize = finalSize * (1.2 + 0.3 * Math.sin(Date.now() * 0.005));
            ctx.globalAlpha = 0.3;
            ctx.beginPath();
            ctx.arc(drawX + CELL_SIZE/2, drawY + CELL_SIZE/2, pulseSize, 0, Math.PI * 2);
            ctx.fill();
            ...
            if (Math.random() < 0.7) {
                ...
                for (let i = 1; i <= tailLength; i++) {
                    const tailSize = finalSize * (1 - i * 0.2);
                    const tailDistance = i * CELL_SIZE * 0.7;
                    const angle = Math.random() * Math.PI * 2;
                    const tailX = drawX + CELL_SIZE/2 - Math.cos(angle) * tailDistance;
                    const tailY = drawY + CELL_SIZE/2 - Math.sin(angle) * tailDistance;
    ...
    constructor(x, y, type) {
        ...
        switch(type) {
            ...
            case 'water':
                ...
                this.energyToReproduce = 85;    // 中程度の繁殖コスト
...
switch(this.type) {
    case 'metal': recoveryChance *= 1.8; break; // 金属は回復しやすい（強化）
    case 'wood': recoveryChance *= 0.4; break;  // 木は回復しにくい（下方修正）
    case 'water': recoveryChance *= 0.3; break; // 水は回復しにくい（下方修正）
    case 'fire': recoveryChance *= 1.3; break;  // 火は回復しやすい
    case 'earth': recoveryChance *= 1.5; break; // 土は回復しやすい（強化）
}
...
if (this.infected <= 1) { // 感染最終段階
    ...
    else if (this.infectedBy === 'lethal') deathChance = 0.6;
    
    // 細胞タイプによる死亡率調整
    if (this.type === 'wood') deathChance *= 2.5; // 木は死亡率が極めて高い
    else if (this.type === 'water') deathChance *= 2.8; // 水は死亡率が極めて高い
...
function triggerMutationEvent() {
    if (!isPandemic && Math.random() < 0.0005) { // 0.05%の確率でイベント発生
        ...
        mutationEventCounter = 3; // 3回の突然変異チャンス
    }
}
...
function update() {
    ...
    // 新しいウイルスを生成（低確率）
    const baseVirusGenerationChance = 0.02; // 基本生成確率を下げる（0.03→0.02）
