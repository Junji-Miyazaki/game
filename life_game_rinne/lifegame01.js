...
let cycleTotalTime = 120;            // 合計サイクル時間（3分 = 180秒）
let growthPhaseTime = 30;            // 成長フェーズ時間（1分）
let peakPhaseTime = 30;              // ピークフェーズ時間（1分）
...
// サイクル管理のための定数
const MIN_CELLS_FOR_GROWTH = 300;     // 成長フェーズに必要な最小細胞数
const CELLS_PEAK_THRESHOLD = 3000;    // ピークフェーズへの移行に必要な細胞数
const CELLS_DECLINE_THRESHOLD = 300; // 減少フェーズの終わりの細胞数しきい値
...
class Virus extends Entity {
    ...
    tryInfection() {
        const neighborCells = getNeighboringCells(this.x, this.y);
        // スケールが小さい(=グリッドが大きい)場合は感染範囲を拡大
    if (BASE_CELL_SIZE < 5 && (this.type === 'mutant' || this.type === 'lethal')) {
        // 追加の周辺セルも取得（広範囲）
...
// グリッドサイズが大きい場合、二次感染の確率と範囲を拡大
if (BASE_CELL_SIZE < 5 && this.infected > 0 && (this.infectedBy === 'lethal' || this.infectedBy === 'mutant')) {
    const gridSizeFactor = Math.max(1, 6 - BASE_CELL_SIZE); // 小さいセルサイズほど大きな係数
    
    // 二次感染の拡散
    const extendedRange = Math.floor(gridSizeFactor * 1.5);
    for (let dx = -extendedRange; dx <= extendedRange; dx++) {
        for (let dy = -extendedRange; dy <= extendedRange; dy++) {
            ...
            if (nx >= 0 && nx < GRID_SIZE_X && ny >= 0 && ny < GRID_SIZE_Y) {
                ...
                if (target instanceof Cell && !target.immunity && !target.infected) {
                    // 距離に応じた感染確率
                    const distance = Math.sqrt(dx*dx + dy*dy);
                    let spreadChance = 0.2 * (extendedRange - distance) / extendedRange;
                    ...
                    if (Math.random() < spreadChance) {
                        target.infected = 20;
                        target.infectedBy = this.infectedBy;
                    }
                }
            }
        }
...
// ...（既存のコード）
            ...
            switch(this.type) {
                ...
                case 'water': 
                    ...
                    break;
                case 'fire': 
				recoveryChance *= cyclePhase === 'decline' ? 1.2 : 1.0; 
				// 減少フェーズでは火の回復率上昇
                    break;
                case 'earth': 
				recoveryChance *= cyclePhase === 'recovery' ? 1.8 : 1.3;
				// 回復フェーズでは土の回復率大幅上昇
                    break;
            }
...
draw() {
    ...
    // 脈動効果を保持
    const pulseFactor = 1 + 0.1 * Math.sin(this.pulsePhase);
    const finalSize = size * pulseFactor;
...
function drawBackground() {
    // フェーズに関係なく固定の背景色を使用
    const innerColor = '#1a1a2e'; // 固定の中心色（暗い青）
    const outerColor = '#0f0f1a'; // 固定の外側色
...
function init() {
	resizeCanvas();
	adjustParametersByGridSize(); // ここでも呼び出す
    entities = [];
    viruses = [];
    cells = [];
    corpses = [];
...
function update() {
    ...
    if (viruses.length < maxViruses && Math.random() < adjustedChance) {
        ...
        if (grid[y][x] === null) {
            ...
            } else if (cyclePhase === 'peak' && Math.random() < 0.15) {
...
window.addEventListener('resize', () => {
    pauseGame();
		resizeCanvas();
		adjustParametersByGridSize(); // 新しい関数を呼び出す
    init();
    draw();
});

// グリッドサイズに基づいて各種パラメータを調整する関数
function adjustParametersByGridSize() {
    // グリッド内のセル総数計算
    const totalCells = GRID_SIZE_X * GRID_SIZE_Y;
    
	// セル数が多いほど感染率を上げる
	/*最大値を 3 から 2.5 に下げました（最大感染率を下げる）
最小値を 1 から 0.8 に下げました（最小感染率も下げる）
除数を 3 から 4 に変更しました（対数スケールの勾配を緩やかにして、グリッドサイズの増加に対する感染率の増加を緩やかにする）*/
    const gridSizeMultiplier = Math.min(2.5, Math.max(1, Math.log10(totalCells) / 4));
    
    // 致死ウイルスのパラメータ調整
    Virus.prototype.baseInfectionRate = 0.25 * gridSizeMultiplier;
    Virus.prototype.baseLethalityRate = 0.15 * gridSizeMultiplier;
	/*
	// 二次関数による緩やかな増加
Virus.prototype.baseInfectionRate = 0.18 + 0.07 * Math.sqrt(gridSizeMultiplier);
Virus.prototype.baseLethalityRate = 0.1 + 0.05 * Math.sqrt(gridSizeMultiplier);
	*/
    // 細胞の成長パラメータも調整
    Cell.prototype.baseReproductionRate = 0.005 / Math.sqrt(gridSizeMultiplier);
    
    console.log(`Grid size adjustment: ${gridSizeMultiplier.toFixed(2)}x multiplier applied`);
}
...
function forceLethalMutation() {
    ...
    const mutationCount = Math.min(viruses.length, Math.floor(viruses.length * 0.05)); // 最大20%
