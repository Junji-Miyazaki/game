// Five Elements Life Game - HTMLとCSSの追加と、JavaScript全体の実装

// 重複コードの問題を解決したファイル
// 以下を「five-elements-life-game-sig_sound.js」として保存し、JavaScriptコードを整理
...
let soundVolume = 0.5; // 50%をデフォルト

// グローバル設定として追加
let useModulation = false; // モジュレーションをオフにするフラグ
let modulationIntensity = 0.5; // モジュレーション強度 (0-1)
let modulationRate = 5; // モジュレーション速度 (Hz)

// 波形タイプのプリセット
const WAVE_PRESETS = {
  pure: { // 純粋な波形のみのプリセット
    wood: { frequency: 1864.66, type: 'sine' },    // B♭6
    fire: { frequency: 2217.46, type: 'sine' },    // C#7/D♭7
    earth: { frequency: 2489.02, type: 'sine' },   // E♭7
    metal: { frequency: 2959.96, type: 'sine' },   // F#7/G♭7
    water: { frequency: 3322.44, type: 'sine' }    // A♭7
  },
  mixed: { // 混合波形プリセット
...
// 現在のサウンドパラメータ
let SOUND_PARAMS = WAVE_PRESETS.pure; // デフォルトは純粋な波形

// ランダムなデチューンとパンを使って音を立体的にする
function getRandomDetune() {
    return (Math.random() * 1) - 0; // ±25セント程度のデチューン
}

function getRandomPan() {
    return (Math.random() * 1) - 0; // -0.8 〜 0.8 の間のパン
}
...
function playDeathSound(cellType) {
    ...
    // パンの設定（ステレオ効果）- モジュレーションなしの場合は中央に固定も可能
    panNode.pan.value = useModulation ? getRandomPan() : 0;
    
    // 音量設定 - シンプルにするか複雑にするかモジュレーション設定による
    if (useModulation) {
        // モジュレーション使用時のエンベロープ
        ...
        // モジュレーションなしの単純なエンベロープ
        //gainNode.gain.setValueAtTime(0.1 * soundVolume, audioContext.currentTime);
		//gainNode.gain.exponentialRampToValueAtTime(0.001 * soundVolume, audioContext.currentTime + 0.01);
		        gainNode.gain.setValueAtTime(soundVolume, audioContext.currentTime);
        gainNode.gain.exponentialRampToValueAtTime(soundVolume, audioContext.currentTime );
    }
    ...
    if (useModulation) {
        ...
        // ビブラート効果
        const vibratoAmount = modulationIntensity * 15; // 0-15セント
...
// 複数の音を少しずらして鳴らす（ジャジーなサウンドインスタレーション的効果）
function playDeathSoundsCluster(cellType, count = 3) {
    if (!isAudioInitialized || !audioContext || !isSoundEnabled) {
        return;
    }
    
    // ジャズのコードのように、音を適切な間隔でずらす
    const jazzDelays = [0, 30, 70, 120, 180]; // ミリ秒単位の非均等なタイミング
    ...
    for (let i = 0; i < count; i++) {
        ...
        setTimeout(() => {
            // 微妙にピッチを変えて、ジャズっぽいハーモニーにする
				//const pitchShift = [-50, 0, 50, 100, 200][Math.floor(Math.random() * 5)];
				 const pitchShift = [-1, 0, 1, 2, 3][Math.floor(Math.random() * 1)];
...
function playDeathSoundWithCustomPitch(cellType, pitchShift) {
    ...
    // 音量設定 - よりジャジーなエンベロープ
   // gainNode.gain.setValueAtTime(0.01 * soundVolume, audioContext.currentTime);
    //gainNode.gain.exponentialRampToValueAtTime(0.15 * soundVolume, audioContext.currentTime + 0.03 + Math.random() * 0.05);
    //gainNode.gain.exponentialRampToValueAtTime(0.001 * soundVolume, audioContext.currentTime + 0.4 + Math.random() * 0.3);
    ...
    // 音を鳴らす - 微妙に長さをランダム化
    oscillator.start();
	//oscillator.stop(audioContext.currentTime + 0.1 + Math.random() * 0.2);
	oscillator.stop(audioContext.currentTime + 0.05 + Math.random() * 0.05);
	 //oscillator.stop(audioContext.currentTime  + 0.05);
}

// 季節変更時用の小さい音を鳴らす関数を追加
function playSeasonChangeSound(seasonType) {
    ...
    // パンの設定（ステレオ効果）
    panNode.pan.value = 0; // 中央に固定
    
    // 音量設定 - 季節変更用に非常に小さく設定
    const seasonChangeVolume = soundVolume * 0.1; // 通常の10%の音量
    gainNode.gain.setValueAtTime(seasonChangeVolume, audioContext.currentTime);
    //gainNode.gain.exponentialRampToValueAtTime(0.001 * seasonChangeVolume, audioContext.currentTime + 0.5);
...
function setupDisasterControls() {
    console.log("Setting up disaster controls");
    ...
    if (triggerDisasterBtn) {
        triggerDisasterBtn.addEventListener('click', function() {
            console.log("Manually triggering disaster...");
            startDisaster();
        });
    }
}
...
// イベントリスナーを追加して、ユーザー操作でオーディオコンテキストを有効化
...
// サウンド関連のカウンター（死亡音が多すぎないようにするため）
let deathSoundCounter = 0;
let deathSoundThreshold = 10; // このターン中に何個の音を鳴らすか（最大）
let deathSoundCooldown = 0;  // クールダウンタイマー
...
window.addEventListener('load', () => {
    // オーディオコンテキストの初期化（ユーザー操作が必要なブラウザに対応）
...
// 以下に残りのコード（calculateCrowdingFactor, Entity, Cell, Virus, etc.）を追加する
    ...
    recoveryChance *= 0.6; // 元: 0.7 → 0.6
            ...
            if (this.infected <= 1) { // 感染最終段階
                ...
                if (Math.random() < deathChance) {
                    ...
                    if (isAudioInitialized && isSoundEnabled && deathSoundCounter < deathSoundThreshold && deathSoundCooldown <= 0) {
                        // 感染死の場合はウイルスタイプに応じて音を調整
                        playDeathSoundsCluster(this.type, 4); // より多くの音を重ねる
                        deathSoundCounter++;
        ...
        if (this.energy < 0 || this.lifespan < 0) {
            ...
            if (isAudioInitialized && isSoundEnabled && deathSoundCounter < deathSoundThreshold && deathSoundCooldown <= 0) {
                ...
                // 種類ごとに音が聞こえるようにする工夫
                if (deathSoundCounter >= deathSoundThreshold) {
                    deathSoundCooldown = 5; // 5ターンのクールダウン
                }
            }
...
// サウンドコントロールを改良して波形プリセットとモジュレーション設定UIを追加
function addSoundControls() {
    // 既存のコントロールパネルを取得
    const controlsPanel = document.getElementById('controls-panel');
    
    // 既存のサウンド設定セクションを削除（もし存在すれば）
    const existingSection = document.getElementById('sound-settings-section');
    if (existingSection) {
        existingSection.remove();
    }
    ...
    soundSection.className = 'control-section';
    soundSection.id = 'sound-settings-section';
    soundSection.innerHTML = `
        <h3>サウンド設定</h3>
        <div class="control-item">
            <label for="soundToggle">サウンド有効:</label>
            <input type="checkbox" id="soundToggle" ${isSoundEnabled ? 'checked' : ''}>
        </div>
        <div class="control-item">
            <label for="soundVolumeSlider">音量:</label>
            <input type="range" id="soundVolumeSlider" min="0" max="100" value="${soundVolume * 100}" step="5">
            <span id="soundVolumeValue">${Math.round(soundVolume * 100)}%</span>
        </div>
        <div class="control-item">
            <label for="waveformSelect">波形タイプ:</label>
            <select id="waveformSelect">
                <option value="pure">純粋な正弦波</option>
                <option value="mixed">混合波形</option>
        ...
        <div class="control-item">
            <label for="modulationToggle">モジュレーション:</label>
            <input type="checkbox" id="modulationToggle" ${useModulation ? 'checked' : ''}>
        </div>
        <div class="control-item">
            <label for="modulationIntensitySlider">モジュレーション強度:</label>
            <input type="range" id="modulationIntensitySlider" min="0" max="100" value="${modulationIntensity * 100}" step="5" ${!useModulation ? 'disabled' : ''}>
            <span id="modulationIntensityValue">${Math.round(modulationIntensity * 100)}%</span>
        </div>
        <div class="control-item">
            <label for="modulationRateSlider">モジュレーション速度:</label>
            <input type="range" id="modulationRateSlider" min="1" max="20" value="${modulationRate}" step="1" ${!useModulation ? 'disabled' : ''}>
            <span id="modulationRateValue">${modulationRate} Hz</span>
        </div>
        <div class="control-item">
            <button id="testSoundButton">音のテスト</button>
        </div>
    `;
    
    // controlsPanelに追加（災害設定の前に）
    const disasterSection = document.getElementById('disaster-settings-section');
    if (disasterSection) {
        controlsPanel.insertBefore(soundSection, disasterSection);
    } else {
        controlsPanel.appendChild(soundSection);
    }
    ...
    document.getElementById('soundVolumeSlider').addEventListener('input', function() {
        soundVolume = parseInt(this.value) / 100;
    ...
    document.getElementById('waveformSelect').addEventListener('change', function() {
        const selected = this.value;
        SOUND_PARAMS = WAVE_PRESETS[selected];
        console.log("波形プリセット変更: " + selected);
    });
    
    // モジュレーションのオンオフ切り替え
    document.getElementById('modulationToggle').addEventListener('change', function() {
        useModulation = this.checked;
        
        // モジュレーション関連スライダーの有効/無効を切り替え
        document.getElementById('modulationIntensitySlider').disabled = !useModulation;
        document.getElementById('modulationRateSlider').disabled = !useModulation;
        console.log("モジュレーション: " + (useModulation ? "オン" : "オフ"));
    });
    
    // モジュレーション強度スライダー
    document.getElementById('modulationIntensitySlider').addEventListener('input', function() {
        modulationIntensity = parseInt(this.value) / 100;
        document.getElementById('modulationIntensityValue').textContent = `${this.value}%`;
    });
    
    // モジュレーション速度スライダー
    document.getElementById('modulationRateSlider').addEventListener('input', function() {
        modulationRate = parseInt(this.value);
        document.getElementById('modulationRateValue').textContent = `${modulationRate} Hz`;
    });
...
// 現在選択されている波形プリセットを反映させるための関数
function updateSoundPresetUI() {
    // 現在のプリセットを特定
    let currentPreset = 'pure'; // デフォルト
    ...
    const waveformSelect = document.getElementById('waveformSelect');
    if (waveformSelect) {
        waveformSelect.value = currentPreset;
    }
}

// モジュレーション設定を反映させるための関数
function updateModulationUI() {
    const modulationToggle = document.getElementById('modulationToggle');
    const intensitySlider = document.getElementById('modulationIntensitySlider');
    const intensityValue = document.getElementById('modulationIntensityValue');
    const rateSlider = document.getElementById('modulationRateSlider');
    const rateValue = document.getElementById('modulationRateValue');
    ...
    if (intensitySlider) {
        intensitySlider.disabled = !useModulation;
        intensitySlider.value = Math.round(modulationIntensity * 100);
    }
    
    if (intensityValue) {
        intensityValue.textContent = `${Math.round(modulationIntensity * 100)}%`;
    }
    
    if (rateSlider) {
        rateSlider.disabled = !useModulation;
        rateSlider.value = modulationRate;
    }
    
    if (rateValue) {
        rateValue.textContent = `${modulationRate} Hz`;
    }
}
...
function startDisaster() {
    if (!disasterActive) {
        ...
        // 情報パネルに災害メッセージを設定
        disasterMessage =// 情報パネルに災害メッセージを設定
        ...
        //if (isAudioInitialized && isSoundEnabled) {
          ...
          //      setTimeout(() => {
          //          playDeathSoundsCluster(CELL_TYPES[i], 1);
          //      }, i * 300);
          //  }
        //}
        */
...
// updateSeason 関数を以下のように修正してください
// ファイル内の updateSeason 関数を探して、音を鳴らす部分のコードを変更します
...
function updateSeason() {
    ...
    if (seasonTimer >= seasonTime) {
        ...
        if (isAudioInitialized && isSoundEnabled) {
            ...
            // 元のコード: playDeathSound(currentSeason);
        }
